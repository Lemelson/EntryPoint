...
let lastGlassRect = { x: 0, y: 0, w: 0, h: 0, visible: true };
function scheduleResultsGlassSync() {
    window.cancelAnimationFrame(resultsGlassRAF);
    resultsGlassRAF = window.requestAnimationFrame(() => syncResultsGlass());
}
...
function mountShell() {
    ...
    <header class="topbar">
      <div class="container topbar-inner">
        <a class="brand" href="#/">
          <span class="brand-mark">ENTRYPOINT</span>
        ...
        <div class="top-actions">
          ...
          <button class="btn" id="show-intro" type="button" title="Показать приветствие">Intro</button>
...
function buildListView(root: HTMLElement) {
    ...
    <section class="container layout">
      <aside class="panel filters" id="filters">
        ...
        <div class="filter-block">
          <h3>Оплата</h3>
          <div class="chips" id="chips-paid">
            ${paidValues.map((p) => chipButton("paid", p, PAID_LABEL[p])).join("")}
          </div>
        </div>
...
function toggleChip(group: string, value: string) {
    ...
    switch (group) {
        ...
        case "fmt":
            ...
            break;
        case "paid":
            toggleSet(f.paid as Set<any>, value);
            break;
        case "city":
...
function isSelected(group: string, value: string): boolean {
    ...
    switch (group) {
        ...
        case "paid":
            return f.paid.has(value as any);
        case "city":
...
function maybeShowWelcome() {
    ...
    <div class="welcome-backdrop" role="dialog" aria-modal="true" aria-label="Добро пожаловать">
      <div class="welcome">
        ...
        <div class="welcome-sub">
          Подбери стажировку по стеку, курсу и городу. Отклик в пару кликов. Компании могут добавить анкету для демо.
        </div>
<ul class="welcome-list">
        ...
        <div class="mini" style="margin-top:10px;">Нажми Esc или кликни по фону, чтобы закрыть.</div>
      </div>
    </div>
  `;
...
function showWelcomeForced() {
    ...
    <div class="welcome-backdrop" role="dialog" aria-modal="true" aria-label="Добро пожаловать">
      <div class="welcome">
        ...
        <ul class="welcome-list">
          <li>Чипы-фильтры: без перезагрузок и "скачков"</li>
          <li>Профиль: GPA и курс (1–4) для отбора по требованиям</li>
          <li>Поделиться ссылкой: копирование в буфер</li>
          <li>Создать анкету: сохраняется локально (для демо)</li>
        </ul>
        <div class="welcome-actions">
          <button class="btn btn-primary" id="welcome-start" type="button">Начать поиск</button>
          <button class="btn" id="welcome-create" type="button">Создать анкету</button>
